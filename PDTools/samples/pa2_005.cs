...
// 
// 
// Assignment: Programming Assignment 2: Great Circile Distances 
// Submission Date: Ocotber 17, 2008 
// 
...
{ 
 ...
 // Initialising the degrees values of latitudes and longitudes 
 // using the parameters latitide and longitude. 
 latitudeDegrees = (int) latitude; 
 longitudeDegrees = (int) longitude; 
 
 //Calculates the minutes values for the latitude and longitude 
 latitudeMinutes = (uint) ((latitude - latitudeDegrees)*60); 
 longitudeMinutes = (uint) ((latitude - latitudeDegrees)*60); 
 
 //Calculates the seconds values for the latitude and longitude 
 latitudeSeconds = (uint) ((((latitude - latitudeMinutes)*60) - 
 latitudeMinutes)*60); 
 longitudeSeconds = (uint) ((((latitude - longitudeMinutes)*60) - 
 longitudeMinutes)*60); 
 ...
 // Calculates and returns a decimal degree value of the latitude 
 get 
 { 
 return (( latitudeDegrees + (( latitudeMinutes + 
 ( latitudeSeconds / 60d )) / 60d))* Math.PI / 180d); 
 } 
 } 
 ...
 // Calculates and returns a decimal degree value of the longitude 
 get 
 { 
 return (( longitudeDegrees + (( longitudeMinutes + 
 ( longitudeSeconds / 60d )) / 60d))* Math.PI / 180d); 
 } 
 } 
 ...
 // Degree values of latitude/longitude 
 string p= " "; 
 
 // Stating the latitude 
 p+= Math.Abs(latitudeDegrees) + "°" +latitudeMinutes + "'" + 
 latitudeSeconds + "\""; 
 // If the latitude degrees value is positive, then the location is in 
 // North, if it was negative, then it's in the South. 
 ...
 // Stating the longitude 
 p+= " "+ Math.Abs(longitudeDegrees) + "°" +longitudeMinutes + "'" + longitudeSeconds + "\""; 
 // If the latitude degrees value is positive, then the location is in 
 // East, if it was negative, then it's in the West. 
...
{ 
 ...
 // Enters the values for the minutes and seconds. 
 uint inputValue; 
 { 
