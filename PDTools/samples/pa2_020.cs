...
// 
// 
// Assignment: <GREAT CIRCLE DISTANCES> 
...
{ 
 ...
 // Assigns the DMS format to each of the data fields 
 latitudeDegrees = (int)latitude; 
 latitudeMinutes = (uint)((3600 * (latitude - (int)latitude)) / 60); 
 latitudeSeconds = (uint)((3600 * (latitude - (int)latitude)) % 60); 
 
 longitudeDegrees = (int)longitude; 
 longitudeMinutes = (uint)((3600 * (longitude - (int)longitude)) / 60); 
 longitudeSeconds = (uint)((3600 * (longitude - (int)longitude)) % 60); 
 ...
 { 
 // The LatitudeDegrees property is not used in Fall 2009 but it is pro 
 // for completeness. 
 ...
 { 
 // The LatitudeMinutes property is not used in Fall 2009 but it is pro 
 // for completeness. 
 ...
 get 
 { 
 // Calculates the Latitude Radians using a specified formula 
 return ((latitudeDegrees + (latitudeMinutes / 60d) + 
 (latitudeSeconds / 3600d)) * (Math.PI / 180d)); 
 } 
 ...
 get 
 { 
 // Calculates the Longitude Radians using a specified formula 
 return((longitudeDegrees + (longitudeMinutes/60d) + 
 (longitudeSeconds/3600d)) * (Math.PI / 180d)); 
 } 
 } 
 ...
 string s, latOrientation, longOrientation; 
 
 // The if statements check to see in what hemishpere the latitude 
 // and longitude degrees are in. In the case that they are in the 
 // South or West hemishpere, the degrees are multiplied by -1 
 // to outpout a positive answer in the correct hemisphere 
 ...
 //outputs the location of the specificed location 
 s = string.Format(" {0}°{1}'{2}\" {3}, {4}°{5}'{6}\" {7}", 
 Math.Abs(latitudeDegrees), latitudeMinutes, latitudeSeconds, 
 latOrientation, Math.Abs(longitudeDegrees), longitudeMinutes, 
 longitudeSeconds,longOrientation); 
...
{ 
 ...
 , max); 
 unsignedValue = uint.Parse(Console.ReadLine()); 
