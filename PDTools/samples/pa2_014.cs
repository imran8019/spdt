...
// Submission Date: <Friday October 16, 2009> 
// 
...
{ 
 ...
 //Converts latitude DMS format to decimal format for the 
 //decimal to radian conversion property 
 latitudeDegrees = (int)latitude; 
 double latm = (latitude - latitudeDegrees) * 60; //used for lat seconds 
 latitudeMinutes = (uint)latm; 
 latitudeSeconds = (uint)(latm - latitudeMinutes) * 60; 
 
 //Converts longitude DMS format to decimal format for the 
 //decimal to radian conversion property 
 longitudeDegrees = (int)longitude; 
 double lonm = (longitude - longitudeDegrees) * 60; //used for lon seconds 
 longitudeMinutes = (uint)lonm; 
 longitudeSeconds = (uint)(lonm - longitudeMinutes) * 60; 
 ...
 //Takes the user input from Main() and assigns 
 //the new object its individual coordinates as specified 
 latitudeDegrees = d1; 
 latitudeMinutes = m1; 
 latitudeSeconds = s1; 
 longitudeDegrees = d2; 
 ...
 //Convert latitude Decimal format to radians 
 //for the distance function 
 get 
 { 
 return (latitudeDegrees + latitudeMinutes / 60.0 + 
 latitudeSeconds / 3600.0) * Math.PI / 180; 
 } 
 } 
 ...
 //Convert longitude Decimal format to radians 
 //for the distance function 
 get 
 { 
 return (longitudeDegrees + longitudeMinutes / 60.0 + 
 longitudeSeconds / 3600.0) * Math.PI / 180; 
 } 
 } 
 ...
 //Initialize variables to determine hemisphere 
 string latdir; 
 string londir; 
 
 //Determine if Latitude is North or South; assumes N for 0 degrees 
 ...
 //Determine if Longitude is East or West; assumes E for 0 degrees 
 ...
 //Outputs the objects Coordinates and Direction to a string 
 string str = " " + Math.Abs(latitudeDegrees) + "\u00b0" 
 + latitudeMinutes + "'" + latitudeSeconds + "\" " 
 + latdir + ", " + Math.Abs(longitudeDegrees) + "\u00b0" 
 + longitudeMinutes + "'" + longitudeSeconds + "\" " + londir; 
...
{ 
 ...
 //Captures user input and compares it the allowable range of 0 - max 
 do 
 { 
 Console.WriteLine("\nEnter the {0} of the {1} ranging", 
 units, measurement); 
 Console.Write("from 0 to {0} as an integer: ", max); 
