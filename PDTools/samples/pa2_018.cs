...
// 
// Acknowledgements: 
// I got help with this assignment from the TA's who do the tutoring sessions. 
// ------------------------------------------------------------------- 
using System; 
...
{ 
 ...
 //Converting latitude from Decimal Degrees format to DMS 
 latitudeDegrees = (int)latitude; 
 latitudeMinutes = (uint)(Math.Abs(latitude - latitudeDegrees) * 60); 
 latitudeSeconds = (uint)(((Math.Abs(latitude - latitudeDegrees)*60) 
 - latitudeMinutes) * 60); 
 
 //Converting longitude from Decimal Degrees to DMS 
 ...
 * 60) - longitudeMinutes) * 60); 
 ...
 //Intializing latitude data fields 
 latitudeDegrees = d1; 
 latitudeMinutes = m1; 
 latitudeSeconds = s1; 
 
 //Intializing longitude data fields 
 longitudeDegrees = d2; 
 longitudeMinutes = m2; 
 longitudeSeconds = s2; 
 } 
 ...
 //Returning latitude from DMS format to decimal format in radians 
 get 
 { 
 if (longitudeDegrees > 0) 
 { 
 return ((latitudeDegrees) + (latitudeMinutes/60.0d) + 
 (latitudeSeconds/3600.0d))*(Math.PI/180); 
 } 
 else 
 { 
 return((latitudeDegrees) - (latitudeMinutes/60.0d) - 
 ...
 //Returning longitude from DMS format to decimal format in radians 
 get 
 { 
 if (longitudeDegrees > 0) 
 { 
 return((longitudeDegrees) + (longitudeMinutes/60.0d) + 
 (longitudeSeconds/3600.0d)) * (Math.PI / 180); 
 } 
 else 
 { 
 return((longitudeDegrees) - (longitudeMinutes/60.0d) - 
 ...
 { 
 latOutput = latitudeDegrees * -1; 
 northOrSouth = 'S'; 
 } 
 else 
 { 
 latOutput = latitudeDegrees; 
 northOrSouth = 'N'; 
 } 
 ...
 if (longitudeDegrees < 0) 
 { 
 lonOutput = longitudeDegrees * -1; 
 eastOrWest = 'W'; 
 } 
 else 
 { 
 lonOutput = longitudeDegrees; 
 eastOrWest = 'E'; 
 } 
 
 
 //returning data fields of the specified location 
 return (string.Format(("{0}\u00b0{1}'{2}\" {3}, ") + 
 ("{4}\u00b0{5}'{6}\" {7}"), latOutput, latitudeMinutes, 
 latitudeSeconds, northOrSouth, lonOutput, 
 longitudeMinutes, longitudeSeconds, eastOrWest)); 
