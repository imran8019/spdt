...
{ 
 ...
 //converts 'latitude' (43.47031587) into degrees, minutes, and seconds then stores it into its corresponding variables 
 ...
 longitudeSeconds = (uint)(((latitude - longitudeDegrees) * 60 - longitudeMinutes) * 60); 
 ...
 // ==== sets the degrees, minutes and seconds to its correspondng variable 
 latitudeDegrees = d1; 
 latitudeMinutes = m1; 
 latitudeSeconds = s1; 
 ...
 get 
 { 
 //convert DMS to deg, convert deg to rads, return rads 
 return (double)((double)(latitudeDegrees) + (double)(latitudeMinutes/60d) + (double)(latitudeSeconds/3600d))* (Math.PI / 180); 
 } 
 } 
 ...
 { 
 //convert DMS to deg, convert deg to rads, return rads 
 return (double)((double)(longitudeDegrees) + (double)(longitudeMinutes / 60d) + (double)(longitudeSeconds / 3600d)) * (Math.PI / 180); 
 } 
 ...
 Math.Abs(latitudeDegrees), latitudeMinutes, latitudeSeconds , latitudeDirection, 
 Math.Abs(longitudeDegrees), longitudeSeconds, longitudeMinutes, longitudeDirection); 
...
{ 
 ...
 int unsignedinputValue; 
 int min = 0;//sets min value to zero 
 ...
 unsignedinputValue = int.Parse(Console.ReadLine()); 
 
 } while (unsignedinputValue < min || unsignedinputValue > max); 
 
 return (uint)(unsignedinputValue); 
 } 
}