...
{ 
 ...
 //changing data to int format. 
 latitudeDegrees = (int)(latitude); 
 
 //must be changed to uint because minutes are positive. 
 latitudeMinutes = (uint)((latitude-latitudeDegrees)*60); 
 
 //must be changed to uint because seconds are positive. 
 ...
 //must be uint because minutes are positive. 
 longitudeMinutes = (uint)((longitude-longitudeDegrees)*60); 
 
 //must be uint because seconds are positive. 
 ...
 // simply defining latitude variables as (int) and (uint) variables. 
 latitudeDegrees = d1; 
 latitudeMinutes = m1; 
 latitudeSeconds = s1; 
 
 // defining longitude variables as (int) and (uint) variables. 
 longitudeDegrees = d2; 
 longitudeMinutes = m2; 
 longitudeSeconds = s2; 
 ...
 // Using get/set to change latitudeDegrees into latR (radian units) 
 get 
 { 
 double latR; 
 double latD; 
 ...
 { 
 // When latitudeDegrees < 0, it's important to subtract 
 // latitudeMinutes and latitudeSeconds from the value. This 
 // way the absolute value of latD will remain the same, no 
 // matter the sign. 
 latD = ((latitudeDegrees - (latitudeMinutes / 60d) 
 - (latitudeSeconds / 3600d))); 
 latR = (latD * Math.PI) / 180d; 
 ...
 { 
 // In the same style as above, when latitudeDegrees > 0, you 
 // add the latitudeMinutes and latitudeSeconds to the total 
 // value of latD. 
 latD = ((latitudeDegrees + (latitudeMinutes / 60d) 
 + (latitudeSeconds / 3600d))); 
 latR = (latD * Math.PI) / 180d; 
 ...
 // This states that the double latR = value so that it can be used. 
 set { double latR = value; } 
 } 
 ...
 // Using get/set to change longitudeDegrees into lonR (radian units) 
 get 
 { 
 double lonR; 
 double lonD; 
 ...
 { 
 // The absolute value of lonD must remain the same no matter 
 // the sign. So when longitudeDegrees < 0, the values must 
 // be subtracted. 
 lonD = ((longitudeDegrees - (longitudeMinutes / 60d) 
 - (longitudeSeconds / 3600d))); 
 lonR = (lonD * Math.PI) / 180d; 
 ...
 { 
 // Similar to above, whenever longitudeDegrees > 0, the values 
 // must be added to insure a correct value of lonD. 
 lonD = ((longitudeDegrees + (longitudeMinutes / 60d) 
 + (longitudeSeconds / 3600d))); 
 lonR = (lonD * Math.PI) / 180d; 
 } 
 ...
 // This sets the double lonR = value so that it can be used later. 
 set { double lonR = value; } 
 } 
 ...
 // This toString method must include four instances, essentially to 
 // match the four hemispheres, SW,SE,NW,NE. This could also be 
 ...
 // Math.abs insures that the value outputted will be positive, 
 // and the sign will relate as a hemisphere (SW). 
 Math.Abs(latitudeDegrees), latitudeMinutes, latitudeSeconds, 
 Math.Abs(longitudeDegrees), longitudeMinutes, longitudeSeconds); 
 } 
 ...
 { 
 // If input = ++, output = NE 
 return string.Format("{0}u00b0{1}'{2}\"N, " + 
 "{3}u00b0{4}'{5}\"E ", 
 
 // No Math.abs is needed here as all values are in the NE 
 // (positive, positive) hemisphere. 
 latitudeDegrees, latitudeMinutes, latitudeSeconds, 
 longitudeDegrees, longitudeMinutes, longitudeSeconds); 
 } 
 } 
...
{ 
 ...
 do 
 { 
 // Just a simple method used to relate user inputted data to the 
