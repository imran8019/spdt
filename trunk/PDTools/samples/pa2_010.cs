...
class GeographicLocation
{
    ...
    public GeographicLocation(double latitude, double longitude)
    {
        ...
        latitudeDegrees=Convert.ToInt32(latitude);
        //convert latitude in degree to int and stores it in latitudeDegrees
        latitudeMinutes=Convert.ToUInt32((latitude-Convert.ToDouble(latitudeDegrees))*60d);
        //calculate latitude minutes and stores as a positive int value in latitudeMinutes
        latitudeSeconds=Convert.ToUInt32((latitude-Convert.ToDouble(latitudeMinutes))*60d);
        //calculate latitude seconds and stores as a positive int value in latitudeSeconds
        longitudeDegrees = Convert.ToInt32(longitude);
        //convert longitude in degree to int and stores it in longitudeDegrees
        longitudeMinutes=Convert.ToUInt32((latitude-Convert.ToDouble(longitudeDegrees))*60d);
        //calculate longitude minutes and stores as a positive int value in longitudeMinutes
        longitudeSeconds=Convert.ToUInt32((latitude-Convert.ToDouble(longitudeMinutes))*60d);
        //calculate longitude seconds and stores as a positive int value in longitudeSeconds
    }
        ...
        uint m2, uint s2)
    {
        ...
        latitudeDegrees=d1;
        //assign latitudeDegrees to d1
        latitudeMinutes=m1;
        //assign latitudeMinutes to m1
        latitudeSeconds=s1;
        //assign latitudeSeconds to s1
        
       longitudeDegrees=d2;
       //assign longitudeDegrees to d2
       longitudeMinutes=m2;
       //assign longitudeMinutes to m2
       longitudeSeconds=s2;
       //assign longitudeSeconds to s2
    ...
    public double LatitudeRadians
    {
        ...
        get
        {
          return (latitudeDegrees+latitudeMinutes*60d+latitudeSeconds*60d)*180/Math.PI;
          // convert the latitude values to decimal degrees convert to pi then return the value
        }
    ...
    public double LongitudeRadians
    {
        ...
        get
        {
          return (longitudeDegrees+longitudeMinutes*60d+longitudeSeconds*60d)*180/Math.PI;
          // convert the longitude values to decimal degrees convert to pi then return the value
        }  
    }
    ...
    public override string ToString()
    {
        ...
        string laDirection;//declare a string
        string loDirection;//declare a string
        ...
        if(longitudeDegrees>0)
          ...
          return (latitudeDegrees)+ "\u00b0" + (latitudeMinutes) + "'" + (latitudeSeconds) + "\"" + (laDirection) + "  "+
          (longitudeDegrees) + "\u00b0" + (longitudeMinutes)+  "'" +(longitudeSeconds) + "\"" +(loDirection);
          //return the DMS value and assign the directions to it
        }
...
class PA2_2009
{
    ...
    static uint getUnsignedInteger(string units, string measurement, uint max)
    {
        ...
        do
        {
            Console.WriteLine("\nEnter the {0} of the {1} ranging",
                units, measurement);
            Console.Write("    from 0\" to {0}\" as an integer: ",
              max);
            inputValue = int.Parse(Console.ReadLine());
        } while (inputValue < 0 || inputValue > max);

        return ((uint)inputValue);
        // return the expected value back to the console
