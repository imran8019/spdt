...
{ 
 ...
 //declaring the variable and making them equal to latitude and 
 //longitude respectively 
 double latitudeDecimalDegrees = latitude; 
 double longitudeDecimalDegrees = longitude; 
 ...
 //declaring what the latitude Deg,Min,Sec represent. d1, m1, s1 respectively. 
 latitudeDegrees = d1; 
 latitudeMinutes = m1; 
 latitudeSeconds = s1; 
 
 //same thing as latitude, but now in longitude. d2, m2, s2, respectively. 
 longitudeDegrees = d2; 
 longitudeMinutes = m2; 
 longitudeSeconds = s2; 
 
 //converting everything (minutes & seconds) into degrees, and adding them. 
 latitudeDecimalDegrees = latitudeDegrees + (latitudeMinutes / 60d) 
 + (latitudeSeconds / 3600d); 
 
 longitudeDecimalDegrees = longitudeDegrees + (longitudeMinutes / 60d) 
 ...
 //convertng the total latitude degrees into radians. 
 get { return latitudeDecimalDegrees * Math.PI /180d; } 
 } 
 ...
 //converting the total longitude degrees into radians. 
 get { return longitudeDecimalDegrees * Math.PI / 180d; } 
 } 
 ...
 //ToString, setting up s and settng up the direction, whether it's 
 //N,E,S,W. 
 
 string s; 
 string directionOfLatitude; 
 string directionOfLongitude; 
 
 //"if" statement so if we want west or south (which can be expressed 
 //with a negative sign) we can put the negative sign, less then 0 
 //and it works. 
 
 if (LatitudeDegrees > 0) 
 { 
 directionOfLatitude = "N"; 
 } 
 else 
 { 
 directionOfLatitude = "S"; 
 } 
 
 if (LongitudeDegrees > 0) 
 { 
 directionOfLongitude = "E"; 
 } 
 else 
 { 
 directionOfLongitude = "W"; 
 } 
 
 //String format so it displays what we want. DD/MM/SS DIR, DD/MM/SS DIR. 
 s = string.Format("{0}°" + "{1}'" + "{2}\" " + "{3}, " + 
 "{4}°" + "{5}'" + "{6}\" " + "{7}", Math.Abs(latitudeDegrees), 
 latitudeMinutes, latitudeSeconds,directionOfLatitude, Math.Abs(longitudeDegrees), 
 longitudeMinutes, longitudeSeconds,directionOfLongitude); 
...
{ 
 ...
 //asking the user to punch in the values desired 
 int unsignedValue; 
 ...
 Console.Write(" from {0}\" to {1}\" as an integer: ", 
 0, max); 
 unsignedValue = int.Parse(Console.ReadLine()); 
 } while (unsignedValue < 0 || unsignedValue > max); 
