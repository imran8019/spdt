...
{ 
 ...
 //Converting latitude to DMS degrees from latitude form 
 latitude = (int)latitudeDegrees; 
 latitudeMinutes = (uint)(latitude - latitudeDegrees) * 60; 
 latitudeSeconds = (uint)(((Math.Abs(latitude - latitudeDegrees) 
 * 60) - latitudeMinutes) * 60); 
 
 //Converting longitude to DMS degrees from longitude form 
 longitude = (int)longitudeDegrees; 
 longitudeMinutes = (uint)(longitude - longitudeDegrees) * 60; 
 longitudeSeconds = (uint)(((Math.Abs(longitude - longitudeDegrees) 
 * 60) - longitudeMinutes) * 60); 
 ...
 get 
 { 
 double latitudeRadian; //storing into a varialbe after conversion 
 
 if (latitudeDegrees > 0) //cheking if it's + or - 
 { 
 
 return ((double)LatitudeDegrees + (double)latitudeMinutes / 60d 
 + (double)latitudeSeconds / 3600d) * Math.PI / 180; 
 } 
 else 
 { 
 latitudeRadian = ((double)latitudeDegrees - ((double) 
 latitudeMinutes / 60 + (double) 
 latitudeSeconds / 3600)) * Math.PI / 180; 
 } 
 ...
 { 
 longitudeRadian = ((double)longitudeDegrees + (double) 
 ...
 * Math.PI / 180; 
 ...
 //declaring a variable for latitude 
 string location; 
 
 
 location = string.Format("{0}\x00B0{1}'{2}\" ", 
 Math.Abs(latitudeDegrees), latitudeMinutes, latitudeSeconds); 
 
 // if less than zero, return south 
 // else north 
 //g = ", "; 
