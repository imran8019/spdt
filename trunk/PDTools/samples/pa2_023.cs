...
// Submission Date: 15th October 2009 
// 
// I declare that, other than the acknowledgements listed below, 
// this program is my original work. 
// 
// Acknowledgements: 
// TA who told me to put "d" (E.g. 60d) when calculating the conversion 
// from DMS to Deciaml Degree format and vice versa. 
// ------------------------------------------------------------------- 
using System; 
...
{ 
 ...
 /* The integer part of Decimal Degree Format "latitude" is 
 the degree in DMS format. Therefore, only taking the int 
 part of latitude */ 
 latitudeDegrees = (int)latitude; 
 //Conversion of decimals of Degrees to Minutes 
 latitudeMinutes = (uint)(60d * (latitude - latitudeDegrees)); 
 //Conversion of decimals of Minutes to Seconds 
 latitudeSeconds = (uint) 
 ( 
 60d * (60d * (latitude - latitudeDegrees) - latitudeMinutes) 
 ); 
 
 longitudeDegrees = (int)longitude; 
 longitudeMinutes = (uint)(60d * (longitude - longitudeDegrees)); 
 longitudeSeconds = (uint) 
 ( 
 60d * (60d * (longitude - longitudeDegrees) - longitudeMinutes) 
 ); 
 } 
 ...
 get 
 { 
 //Conversion of DMS to Decimal format 
 ...
 // string ns and ew denoting North, South, East, West 
 string ns; 
 string ew; 
 
 //Conditions determining North/South hemisphere 
 if ((latitudeDegrees + latitudeMinutes / 
 60d + latitudeSeconds / 3600d) >= 0) 
 { 
 ns = "N"; 
 } 
 ...
 //Conditions determining East/West hemisphere 
 if ((longitudeDegrees + longitudeMinutes / 
 60d + longitudeSeconds / 3600d) >= 0) 
 { 
 ew = "E"; 
 } 
 ...
 ( 
 //DMS format for latitude and longitude 
 "{0}\u00b0{1}'{2}\" {3}, {4}\u00b0{5}'{6}\" {7}", 
 
 //Absolute value to avoid displaying negative degrees 
 Math.Abs(latitudeDegrees), 
 latitudeMinutes, latitudeSeconds, ns, 
 Math.Abs(longitudeDegrees), 
 longitudeMinutes,longitudeSeconds, ew 
 ); 
 } 
 } 
...
{ 
 ...
 max); 
 uinputValue = uint.Parse(Console.ReadLine()); 
 } while (uinputValue > max); 
