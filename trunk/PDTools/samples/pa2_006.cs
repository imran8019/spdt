...
{ 
 ...
 //Converts DD format to DMS format 
 ...
 public GeographicLocation(int d1, uint m1, uint s1, int d2, uint m2, uint 
 s2) 
 { 
 // This constructor should initialize all data fields contained 
 // within an instance of a GeographicLocation object using the 
 ...
 (latitudeSeconds / 60.0)) / 60.0) * Math.PI / 180.0; //Gets value 
 } 
 } 
 ...
 (longitudeSeconds / 60.0)) / 60.0) * Math.PI / 180.0; //Gets value 
 } 
 } 
 ...
 { 
 lat = "N"; //North hemisphere 
 } 
 else if (latitudeDegrees < 0) 
 { 
 lat = "S"; //South hemisphere 
 } 
 else 
 { 
 lat = ""; 
 ...
 { 
 lon = "E"; //East hemisphere 
 } 
 else if (longitudeDegrees < 0) 
 { 
 lon = "W"; //West hemisphere 
 } 
 else 
 { 
 lon = ""; 
 ...
 Math.Abs(latitudeDegrees), latitudeMinutes, latitudeSeconds, lat, 
 Math.Abs(longitudeDegrees), longitudeMinutes, longitudeSeconds, lon); 
...
{ 
 ...
 max); 
 inputValue = uint.Parse(Console.ReadLine()); //Accepts user input 
 //Makes sure usre input is within specified range 
