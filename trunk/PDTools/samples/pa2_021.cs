...
// 
// Acknowledgements: 
// Thank you to William Bishop for his assistance with the first 
// constructor. His insight was helpful in the completion of this 
// project. 
// Mohammed Babadi, for his suggestions and advice on constructing 
// the latitudeRadians and Longitude Radians method. 
// 
// NOTE: This is my original code, and the acknowledgements are for 
// the reader's understading as to the source of technique or coincidental 
// similarities to someone else's code or coding style. 
// ------------------------------------------------------------------- 
using System; 
...
{ 
 ...
 // added for the benfit and simplicity of the property statements 
 // LatitudeRadians and LongitudeRadians 
 private double latituderadians; 
 private double longituderadians; 
 
 /// constructor below is written, but never used by the program, 
 /// as there is no object that is declared which utilizes 
 /// this constructor 
 ...
 latitudeSeconds = (uint)(((latitude - latitudeDegrees) 
 * 60 - latitudeMinutes) * 60); 
 Console.WriteLine("{0} , {1} , {2} ", 
 latitudeDegrees, latitudeMinutes, LatitudeSeconds); 
 } 
 
 public GeographicLocation(int d1, uint m1, uint s1, 
 int d2, uint m2, uint s2) 
 { 
 
 // Sets variables from Main() to the given variables in this class 
 ...
 //if/else logic added so that the positive double values of latitudeMinutes 
 // and latitudeSeconds will be added correctly for a negative degrees value 
 ...
 { 
 return latituderadians = ((double)(latitudeDegrees + (double) 
 latitudeMinutes / 60 + (double)latitudeSeconds / 3600)) * 
 Math.PI / 180; 
 } 
 else 
 { 
 ...
 if (latitudeDegrees > 0) 
 { 
 return longituderadians = ((double)(longitudeDegrees + (double) 
 longitudeMinutes / 60 + (double)longitudeSeconds / 3600)) * 
 Math.PI / 180; 
 } 
 else 
 { 
 ...
 // there is if/else logic inserted to differentiate between the hemispheres 
 // on the map and North and South directions 
 ...
 latitudeDegrees, latitudeMinutes, latitudeSeconds, longitudeDegrees, 
 longitudeMinutes, longitudeSeconds); 
 } 
 else 
 { 
 return string.Format("{0}\u00b0{1}'{2} N {3}\u00b0{4}'{5} W", 
 latitudeDegrees, latitudeMinutes, latitudeSeconds, longitudeDegrees * -1, 
 longitudeMinutes, longitudeSeconds); 
 } 
 } 
 else 
 ...
 if (longitudeDegrees > 0) 
 { 
 return string.Format("{0}\u00b0{1}'{2} S {3}\u00b0{4}'{5} E", 
 latitudeDegrees * -1, latitudeMinutes, latitudeSeconds, 
 longitudeDegrees,longitudeMinutes, longitudeSeconds); 
 } 
 else 
 { 
 return string.Format("{0}\u00b0{1}'{2} S {3}\u00b0{4}'{5} W", 
 latitudeDegrees * -1, latitudeMinutes, latitudeSeconds, 
 longitudeDegrees * -1, longitudeMinutes, longitudeSeconds); 
 } 
 } 
...
{ 
 ...
 do 
 { 
 Console. WriteLine("\n Input the{0} of the {1} ranging", units, measurement); 
 Console.Write (" from 0 to 60 as an integer: "); 
